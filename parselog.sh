...
#grep 'bbox_loss =' aux.txt | awk '{print $8}' > aux1.txt
#grep 'cls_loss =' aux.txt | awk '{print $8}' > aux2.txt
...
grep ', loss = ' $1 >> aux.txt
grep ' bbox_loss = ' $1 >> aux.txt
grep ' cls_loss = ' $1 >> aux.txt
grep ' rpn_loss_bbox = ' $1 >> aux.txt
grep ' rpn_cls_loss = ' $1 >> aux.txt

grep 'Iteration ' aux.txt | sed  's/.*Iteration \([[:digit:]]*\).*/\1/g' > aux0.txt
grep ', loss = ' $1 | awk '{print $9}' > aux1.txt
grep ', lr = ' $1 | awk '{print $9}' > aux2.txt
grep ' bbox_loss = ' $1 | awk '{print $11}' > aux4.txt
grep ' cls_loss = ' $1 | awk '{print $11}' > aux5.txt
grep ' rpn_loss_bbox = ' $1 | awk '{print $11}' > aux6.txt
grep ' rpn_cls_loss = ' $1 | awk '{print $11}' > aux7.txt

# Extracting elpased seconds
./extract_seconds.py aux.txt aux3.txt

# Generating
echo '# Iters Seconds TrainingLoss LearningRate BboxLoss ClsLoss RpnBboxLoss RpnClsLoss'> $LOG.train
paste aux0.txt aux3.txt aux1.txt aux2.txt aux4.txt aux5.txt aux6.txt aux7.txt | column -t >> $LOG.train
